...
It is a simple, yet useful, web browser addon/extension that takes a selected URL via context-menu and submits it directly to be checked against all virus and malware engines on VirusTotal with an added feature that VirusTotal doesn't have. This added feature is basically the part of the process that tries to find the effective URL (redirect) if it exists on the URL that was submitted, and sends that URL to VirusTotal rather than the URL that was selected. This extra step increases the chances of allowing the user to see VirusTotal results for the URL/domain that they would have ended up at, as opposed to the original link. URL redirection (hiding the actual URL) is a major phishing and malware distribution method that attackers use by leveraging legitimate websites.
