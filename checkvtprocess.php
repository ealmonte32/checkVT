...
if (empty($parse['scheme'])) {
    ...
    //echo "<script>alert('debug: vtfinal: $vtfinal_url');</script>";
    //echo "<script>alert('debug: vtfinal analysis: $vtfinal_url_analysis');</script>"; 

	// Insert the URL into the database
	$query = 'INSERT INTO urls_processed (URL_Orig_Address, URL_Final_Address, URL_Processed_Date) VALUES (:incoming_url, :vtfinal_url, :process_timestamp)';
    $statement = $db->prepare($query) or die ("Failed to prepare statement!");
    $statement->bindValue(':incoming_url', $var_incoming_url);
    $statement->bindValue(':vtfinal_url', $vtfinal_url_analysis);
    ...
    //for debugging purposes return to index page
